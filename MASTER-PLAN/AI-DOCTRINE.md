# Casedra — AI Doctrine

**Date:** 2026-04-18  
**Purpose:** define how Casedra should build AI so it becomes durable infrastructure, not a short-lived feature company

---

## Why This Exists

The breakout AI application companies in law, healthcare, and customer service did not win because they looked the most magical.

They won because they became:

- embedded in the daily workflow
- trusted in high-stakes moments
- hard to replace operationally
- stronger with every customer deployment

Casedra should use the same playbook.

The goal is not to become "an AI startup for real estate."
The goal is to become the professional operating standard for agency revenue workflows in Spain, then Portugal and Italy.

---

## 1. Sell Workflow, Not AI

Customers should buy Casedra because it helps them:

- answer leads faster
- recover missed demand
- route conversations correctly
- win more seller instructions
- attach partner revenue more intelligently

They should not buy Casedra because:

- the prompts look clever
- the outputs sound futuristic
- the media demo is impressive

If the workflow value is weak, the AI story is cosmetic.

---

## 2. Live Where The Work Already Happens

The winning AI products do not ask professionals to leave their real workflow.

For Casedra, the real workflow is:

- WhatsApp
- portal lead forwarding
- the team inbox
- manager review
- seller follow-up

If Casedra requires a parallel habit before it creates value, adoption will slow and conviction will drop.

---

## 3. Trust Architecture Before Autonomy

Casedra should never confuse autonomy with value.

The system must make it obvious:

- what the AI did
- when a human owns the conversation
- why a handoff happened
- what the next action is
- when confidence is too low to continue

The product should feel safe, legible, and accountable before it feels impressive.

---

## 4. Forward Deployment Is Product

The first 200 accounts should not be treated like customers buying finished software.

They are how Casedra learns:

- setup friction
- routing edge cases
- trust breakers
- seller-workflow gaps
- benchmark patterns

That means onboarding, deployment, and customer enablement are not support functions.
They are part of the moat.

---

## 5. Evaluations Are A Shipping Requirement

Every serious AI workflow company develops an evaluation discipline.

Casedra should ship AI only when the team can measure:

- response quality
- escalation quality
- handoff quality
- parsing accuracy
- seller-playbook reliability

No important workflow should depend on prompt intuition alone.

---

## 6. The Benchmark Network Is A Core Asset

Casedra should instrument the workflow so it can understand:

- response times by agency and city
- lead-source quality
- conversation outcomes
- seller-capture performance
- instruction-win patterns
- partner attach patterns

This data should not sit in the background.
It should become product, reporting, GTM proof, and strategic leverage.

---

## 7. Productize The Best Teams' Playbooks

The best agencies will teach Casedra how top performers:

- reply
- qualify
- route
- follow up
- present performance to sellers

Those patterns should become reusable product behavior.

The long-term product is not just software.
It is software plus the best operational playbooks in the market.

---

## 8. Keep Model Optionality, Own The Workflow

Casedra should assume frontier models will improve fast and become easier to access.

So the moat is not:

- one model provider
- one prompt set
- one flashy capability

The moat is:

- workflow ownership
- structured data
- benchmark insight
- evaluation loops
- deployment knowledge

Model providers should stay replaceable whenever possible.

---

## 9. Build Rituals, Not Just Features

Enduring software becomes a habit.

Casedra should create rituals such as:

- the daily inbox review
- the weekly manager performance memo
- the weekly seller-proof pack
- the standard listing presentation
- the benchmark every serious agency checks

That is how product becomes culture.

---

## 10. Become The Professional Standard

Idealista became part of market behavior because it became a default reference point.

Casedra should aim to define:

- what "fast enough" response looks like
- what a professional handoff looks like
- what a healthy agency inbox looks like
- what credible seller proof looks like
- what good owner follow-up looks like

If Casedra sets the standard, it becomes harder to displace than a normal software vendor.

---

## What This Means Right Now

### Product

- keep `Responde + Inbox` as the wedge
- build trust, evaluations, and reporting before broader autonomy
- make benchmark surfaces and proof artifacts part of the roadmap

### GTM

- sell recovered revenue and seller wins
- lead with audits and deployments
- turn weekly proof into the commercial engine

### Team

- keep developers close to field feedback
- treat deployment insights as roadmap input
- reward reliability and usage, not demo novelty

### Brand

- speak like the standard for professional agency operations
- avoid generic "AI magic" positioning
- make Casedra feel calm, precise, and authoritative

---

## The One Line To Remember

**Casedra should build AI the way category-defining workflow companies do: embedded, trusted, deployment-heavy, benchmark-led, and hard to replace.**
